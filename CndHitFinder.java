...
public class CndHitFinder {
	...
	// This class contains the core of the code. The findhits method reconstruct
	// good cnd hits from raw halfhits using various cuts and matching.
	// The following method are used to calculate the length the particle is
	// travelling in the paddle while depositing energy.

	// flag to distinguish between calibration mode 0 (loose cuts) and
	// reconstruction mode 1 (tighter cuts)
	...
	public ArrayList<CndHit> findHits(ArrayList<HalfHit> halfhits, int flag, CalibrationConstantsLoader ccdb) {
		...
		if (halfhits.size() > 0) {
			...
			// Loop through the half-hits array to find possible physical combinations with
			// neighbours.

			double matchingchi2 = 9999.9; // chi2 of the matching of the two half-hits

			double E1 = 0;// used to check that the two component of the deposited energy are roughtly the
							// same
			double E2 = 0;
			...
			double expectedTdown = 0.; // Expected time at downstream end of hit paddle (calculated from the
										// direct signal)
			double Eup = 0.; // Energy at upstream end of hit paddle
			double Edown = 0.; // Energy at downstream end of hit paddle

			double Z_dh_av = 0.; // Z of the hit position (local co-ordinates, wrt the centre of the paddle) as
									// supposed for a double hit
			double z_dh_hit = 0.; // Reconstructed position of the particle hit in the paddle (global
									// co-ordinates, wrt to centre of the Central Detector) as supposed for a double
									// hit
			...
			double z_hit = 0.; // Reconstructed position of the particle hit in the paddle (global
								// co-ordinates, wrt to centre of the Central Detector)
			double x_hit = 0.; // Reconstructed position of the particle hit in the paddle (global
								// co-ordinates, wrt to centre of the Central Detector)
			double y_hit = 0.; // Reconstructed position of the particle hit in the paddle (global
								// co-ordinates, wrt to centre of the Central Detector)
			double E_hit = 0; // Reconstructed energy deposit of the particle in the paddle
			double r_hit = 0.; // Perpendicular distance of the hit position (assuming center of paddle) from
								// the beam-axis
			double path = 0.; // path length travelled by particle (assuming a straight line)
			double phi_hit = 0.; // Phi angle of the hit position (assuming center of paddle) from the
									// x-axis(9o'clock looking downstream)
			...
			for (int i = 0; i < (halfhits.size()); i++) {
				...
				lay = hit1.Layer();
				if (pad == 1)
					neigh = 2; // the neighbouring paddle
				else
					neigh = 1;

				// Now loop through the half-hits again and match any which can give a physical
				// reconstruction,
				// but off-set the start of the list to make sure no repeats:

                //reintialize the chi2 after each match.

				for (int j = i + 1; j < halfhits.size(); j++) {
					HalfHit hit2 = halfhits.get(j); // get the second half-hit

					if (block != hit2.Sector())
						continue; // half-hits must be in the same sector
					if (lay != hit2.Layer())
						continue; // half-hits must be in the same layer
					if (hit2.Component() != neigh)
						continue; // half-hits must come from coupled paddles

					// Decide which one of the two signals is the direct and which one is indirect
					...
					double delta = (ccdb.LENGTH[lay - 1] / 10.)
							* ((1. / ccdb.EFFVEL[block - 1][lay - 1][0]) - (1. / ccdb.EFFVEL[block - 1][lay - 1][1]));
					double deltaR;

					if (hit1.Component() == 1)
						deltaR = hit1.Tprop() - hit2.Tprop();
					else
						deltaR = hit2.Tprop() - hit1.Tprop();

					// System.out.println("sector "+block+" layer "+lay+" v L
					// "+CalibrationConstantsLoader.EFFVEL[block-1][lay-1][0]+ " v R
					// "+CalibrationConstantsLoader.EFFVEL[block-1][lay-1][1]+ " delta "+delta + "
					// deltaR "+deltaR );
					...
					// Now calculate the time and energy at the upstream and downstream ends of the
					// paddle the hit happened in:
					// attlen is in cm. need to convert to mm -> *10
					Tup = hit_d.Tprop();
					Tdown = hit_n.Tprop()
							- ccdb.LENGTH[lay - 1] / (10. * ccdb.EFFVEL[block - 1][lay - 1][hit_n.Component() - 1])
							- ccdb.UTURNTLOSS[block - 1][lay - 1];

					expectedTdown = ccdb.LENGTH[lay - 1]
							/ (10. * ccdb.EFFVEL[block - 1][lay - 1][hit_n.Component() - 1])
							+ ccdb.UTURNTLOSS[block - 1][lay - 1]
							+ ccdb.LENGTH[lay - 1] / (10. * ccdb.EFFVEL[block - 1][lay - 1][hit_n.Component() - 1])
							- Tup;
					matchingchi2 = Math.pow((Tdown - expectedTdown) / Parameters.Tres, 2);

					// Eup =
					// hit_d.Eatt()/CalibrationConstantsLoader.MIPDIRECT[block-1][lay-1][hit_d.Component()-1];
					// Edown =
					// hit_n.Eatt()/(Math.exp(-1.*CalibrationConstantsLoader.LENGTH[lay-1]/(10.*CalibrationConstantsLoader.ATNLEN[block-1][lay-1][hit_n.Component()-1]))*CalibrationConstantsLoader.UTURNELOSS[block-1][lay-1]*CalibrationConstantsLoader.MIPDIRECT[block-1][lay-1][hit_n.Component()-1]);

					// The next two lines have to be used if want to use MIP Indirect for
					// reconstruction
					Eup = hit_d.Eatt() / ccdb.MIPDIRECT[hit_d.Sector() - 1][hit_d.Layer() - 1][hit_d.Component() - 1];
					Edown = hit_n.Eatt()
							/ ccdb.MIPINDIRECT[hit_d.Sector() - 1][hit_d.Layer() - 1][hit_d.Component() - 1];

					// For this particular combination, check whether this gives a z within the
					// paddle length (+/- z resolution).
					...
					// the event:
					T_hit = (Tup + Tdown
							- (ccdb.LENGTH[lay - 1] / (10. * ccdb.EFFVEL[block - 1][lay - 1][hit_d.Component() - 1])))
							/ 2.; // time of hit in the paddle
					...
					if (flag == 1) {
						if ((T_hit) < Parameters.MinTime[lay - 1] || (T_hit) > Parameters.MaxTime[lay - 1])
							continue;
					}

					// Calculate the deposited energy and check whether it's over the imposed
					// threshold.
					E_hit = (Eup / Math.exp(-1. * (ccdb.LENGTH[lay - 1] / 2. + Z_av)
							/ (10. * ccdb.ATNLEN[block - 1][lay - 1][hit_d.Component() - 1])))
							+ (Edown / Math.exp(-1. * (ccdb.LENGTH[lay - 1] / 2. - Z_av)
									/ (10. * ccdb.ATNLEN[block - 1][lay - 1][hit_d.Component() - 1])));

					// test (check if the two component of the energy are roughtly the same)
					E1 = (Eup / Math.exp(-1. * (ccdb.LENGTH[lay - 1] / 2. + Z_av)
							/ (10. * ccdb.ATNLEN[block - 1][lay - 1][hit_d.Component() - 1])));
					E2 = (Edown / Math.exp(-1. * (ccdb.LENGTH[lay - 1] / 2. - Z_av)
							/ (10. * ccdb.ATNLEN[block - 1][lay - 1][hit_d.Component() - 1])));
					...
					if (flag == 1) {
						if (E_hit < Parameters.EThresh)
							continue;
					}

					// third cut (added by Pierre) : the energy of both component of the energy have
					...
					if (hit_d.Component() == 1)
						phi_hit = (block - 1) * Parameters.BlockSlice + 0.25 * Parameters.BlockSlice;
					else if (hit_d.Component() == 2)
						phi_hit = (block - 1) * Parameters.BlockSlice + 0.75 * Parameters.BlockSlice;

					// in mm
					z_hit = (((-1. * ccdb.ZOFFSET[lay - 1]) + (ccdb.LENGTH[lay - 1] / 2.)) + Z_av)
							+ (ccdb.ZTARGET[0] * 10); // z co-ordinate of hit in the paddle wrt Central Detector centre
					...
					Z_dh_av = ((ccdb.LENGTH[lay - 1] / (10. * ccdb.EFFVEL[block - 1][lay - 1][hit_n.Component() - 1])
							+ ccdb.UTURNTLOSS[block - 1][lay - 1]) * 10.
							* ccdb.EFFVEL[block - 1][lay - 1][hit_d.Component() - 1]) / 2.;
					z_dh_hit = (((-1. * ccdb.ZOFFSET[lay - 1]) + (ccdb.LENGTH[lay - 1] / 2.)) + Z_dh_av)
							+ (ccdb.ZTARGET[0] * 10);
					//

					// Create a new CndHit and fill it with the relevant info:

					CndHit GoodHit = new CndHit(pad_d, pad_n); // Takes as index the halfhits array indices of the two
																// half-hits involved.

					GoodHit.set_Chi2(matchingchi2); // set the chi2 of the matching
					...
					if (Math.abs(z_dh_hit - z_hit) / 10 < ccdb.DeltaZDH[lay - 1]
							&& Math.abs(hit_d.Tprop() - hit_n.Tprop()) < ccdb.DeltaTDH[lay - 1]) {
			...
			// At this stage an array of possible reconstructed hits, type CndHit and called
			// HitArray, has been created.
			// There may be cases of ambiguous reconstruction: where two signals from one
			// paddle can be matched up with a single one from the neighbour.
			...
			for (int i = 0; i < (HitArray.size()); i++) {
				...
				for (int j = i + 1; j < HitArray.size(); j++) {
					...
					if (i != j) 
					{
						if ((cndhit1.index_d() == cndhit2.index_d() || cndhit1.index_d() == cndhit2.index_n()
								|| cndhit1.index_n() == cndhit2.index_d() || cndhit1.index_n() == cndhit2.index_n())) 
								{
									if(cndhit1.Chi2() < cndhit2.Chi2()) 
									{ // check chi2
							         ambig_rec[j] = 1; // set the flags for ambiguous reconstructions
									}
									else if(cndhit1.Chi2() > cndhit2.Chi2()) 
									{
							         ambig_rec[i] = 1;
						            }
						            else{
								...
								else if (cndhit1.Sector() == cndhit2.Sector()
										&& cndhit1.Layer() == cndhit2.Layer()) {
									if (cndhit1.Chi2() < cndhit2.Chi2()) {
										ambig_rec[j] = 1; // set the flags for ambiguous reconstructions
									} 
									else  
			...
			// Now loop through and create a new array in which there are no ambiguous
			// reconstructions:
			for (int i = 0; i < HitArray.size(); i++) {
			...
			// Sort the hits in order of ascending time, so the first one has the shortest
			// time:
	...
	public double findLength(CndHit hit, List<CVTTrack> helices, int flag, CalibrationConstantsLoader ccdb) {
		// this method is used to find the length of the path followed by the detected
		// charged particle in the cnd
		// first we need to know if the particle is charged by matching the hit to the
		// cvt helical tracks
		...
		double zi = hit.Z();
		int lay = hit.Layer();
		// Constants SHOULD NOT be in the code methods BUT in a parameters file or in
		// CCDB!!!
		...
		// double incx =
		// Math.sqrt(((xi*xi*15.*15.)/(radius*radius))+((yi*yi)*(3.75*3.75*(Math.PI/180.)*(Math.PI/180.))));
		// //15 is half the paddle thickness
		// double incy =
		// Math.sqrt(((yi*yi*15.*15.)/(radius*radius))+((xi*xi)*(3.75*3.75*(Math.PI/180.)*(Math.PI/180.))));
		// // 3.75 is the incertainty in phi
		// double incz = 38.4/Math.sqrt(hit.Edep()); //evaluated with the non-corrected
		// energy

		double incx = Math.sqrt(((xi * xi * Rres * Rres) / (radius * radius))
				+ ((yi * yi) * (Phires * Phires * (Math.PI / 180.) * (Math.PI / 180.))));
		double incy = Math.sqrt(((yi * yi * Rres * Rres) / (radius * radius))
				+ ((xi * xi) * (Phires * Phires * (Math.PI / 180.) * (Math.PI / 180.))));
		double incz = (10. * ccdb.EFFVEL[hit.Sector() - 1][lay - 1][hit.Component() - 1] * Parameters.Tres)
				/ Math.sqrt(hit.Edep());
		...
		for (int i = 0; i < helices.size(); i++) {// loop through helical tracks extracted from the cvt
			...
			// the following line give the point at which the cnd hit would occur according
			// to the cvt track
			...
			if (hitCndfromCvtList.isEmpty())
				continue; // Check if a layer as a swimmer intersection. If not go to the next track
			...
			// if(Math.abs(xi-xj)<(3.*incx) && Math.abs(yi-yj)<(3.*incy) &&
			// Math.abs(zi-zj)<(3.*incz)) { // CUT are set to x,y,z incertainty
			...
			if ((flag == 0 && Math.abs(xi - xj) < (5. * incx) &&
					...
					(flag == 1 && Math.abs(xi - xj) < (5. * incx) && Math.abs(yi - yj) < (5. * incy) &&
							Math.abs(zi - zj) < (5. * incz))) { // CUT are set to x,y incertainty and zj in the paddle
																// length
				...
				// get the length travelled by the particule in the paddle. If the entry point
				// is outside the cnd skip the event, if the escape point is outside the paddle,
				// the escape point is the intersection of the particule path and the plane
				// defined at the edge of the paddle, otherwise get the distance between
				// entrypoint and escape point.

				if (helices.get(i).get_TrkInters().get(lay - 1).get(0)
						.z() > ((-1. * ccdb.ZOFFSET[lay - 1]) + (ccdb.LENGTH[lay - 1])))
					continue;
				if (helices.get(i).get_TrkInters().get(lay - 1).get(0).z() < (-1. * ccdb.ZOFFSET[lay - 1]))
					continue;

				Plane3D zmax = new Plane3D(0.0, 0.0, ((-1. * ccdb.ZOFFSET[lay - 1]) + (ccdb.LENGTH[lay - 1])), 0.0, 0.0,
						1.0);
				Plane3D zmin = new Plane3D(0.0, 0.0, (-1. * ccdb.ZOFFSET[lay - 1]), 0.0, 0.0, 1.0);

				if (helices.get(i).get_TrkInters().get(lay - 1).get(2)
						.z() > ((-1. * ccdb.ZOFFSET[lay - 1]) + (ccdb.LENGTH[lay - 1]))) {
					Line3D ray = new Line3D(helices.get(i).get_TrkInters().get(lay - 1).get(0),
							helices.get(i).get_TrkInters().get(lay - 1).get(2));
					Point3D inter = new Point3D();
					if (zmax.intersection(ray, inter) != 1)
						continue;
				...
				if (helices.get(i).get_TrkInters().get(lay - 1).get(2).z() < (-1. * ccdb.ZOFFSET[lay - 1])) {
					Line3D ray = new Line3D(helices.get(i).get_TrkInters().get(lay - 1).get(0),
							helices.get(i).get_TrkInters().get(lay - 1).get(2));
					Point3D inter = new Point3D();
					if (zmin.intersection(ray, inter) != 1)
						continue;
					length = helices.get(i).get_TrkInters().get(lay - 1).get(0).distance(inter);
					// System.out.print("use inter - "+inter.z());
				}
					...
					length = helices.get(i).get_TrkInters().get(lay - 1).get(0)
							.distance(helices.get(i).get_TrkInters().get(lay - 1).get(2));
				}
	...
	public double findLengthNeutral(Point3D vertex, CndHit hit, CalibrationConstantsLoader ccdb) {
		...
		// if the particle is not charged, it is not detected in the cvt and we can
		// calculate its pathlength using reconstructed vertex
		double length = 0.;
		...
		if (vertex != null) {
			...
			// get the length of the path as the distance between the two intersection
			// points of
			// the line between the hit point and the vertex and 2 cylinders corresponding
			// to the hit paddle.
			...
			// the cylinders are biggers than the actual cnd but it is just for convenience,
			// as the hit point is in the cnd anyway
			...
			if (entrypoints.size() == 1 && exitpoints.size() == 1) {
				...
				System.err.println("probleme intersection" + " entrypoints nb " + entrypoints.size() + " exitpoints nb "
						+ exitpoints.size());
			}
