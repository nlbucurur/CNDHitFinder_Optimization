...
public class CndHit extends ArrayList<HalfHit> implements Comparable<CndHit> {
	...
	/**
	 * A CndHit consists of an array of particle hits in the CND reconstructed
	 * according to
	...
	private double _chi2; // chi2 of matching two half-hits
	...
	private int _indexLadc; // index to match row adc and tdc to reconstructed hit (index L (resp.R) should
							// be the same for adc and tdc)
	private int _indexRadc;
	private int _indexLtdc; // index to match row adc and tdc to reconstructed hit (index L (resp.R) should
							// be the same for adc and tdc)
	...
	public void set_Chi2(double mchi2) {
		this._chi2 = mchi2;
	}
